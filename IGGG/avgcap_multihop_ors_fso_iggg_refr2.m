...
for m = 1:2
    ...
    for i = 1:length(Eb_N0_dB)
        const_1 = (2^(alpha+beta+lambda_a-k-4))/((pi^1.5)*log(2));
        const_2 = ((v^k)*chi)/zeta;
        arg(i) = ((zeta^2)*(B^2))/(4*s_r*Eb_N0(i));
        cap_t(i) = const_1*const_2*meijerG([(1-lambda_a)/2,(2-lambda_a)/2,0],[1,(1+g^2)/2,(2+g^2)/2,(1+v)/2,(1+v)/2,(2+v)/2,(2+v)/2,],[(g^2)/2,(1+g^2)/2,alpha/2,(1+alpha)/2,beta/2,(1+beta)/2,v/2,v/2,(1+v)/2,(1+v)/2,0,0],[],arg(i));
    end
    cap_t_final(m,:) = cap_t;
end
...
for m = 1:2
    ...
    for i = 1:length(Eb_N0_dB)
        for j = 1:N
            h_a = gamrnd(alpha,1/alpha,1,M).*gamrnd(beta,1/beta,1,M).*(1./gamrnd(lambda_a,1/(lambda_a-1),1,M)); % Atmospheric turbulance (IGGG distribution)
...
figure
semilogy(Eb_N0_dB,cap_s_oall_final(1,:),'pentagram',Eb_N0_dB,cap_s_oall_final(2,:),'v','linewidth',3,'markersize',8);
hold on
semilogy(Eb_N0_dB,cap_t_final,'k-','lineWidth',1.5);
legend('Strong Refractive Index','Moderate Refractive Index','Theoretical','Location','southeast')
grid on
