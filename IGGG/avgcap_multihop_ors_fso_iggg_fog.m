...
for m = 1:3
    ...
    for i = 1:length(Eb_N0_dB)
        ...
        cap_t(i) = const_1*const_2*meijerG([(1-lambda_a)/2,(2-lambda_a)/2,(1-lambda_a)/2,(2-lambda_a)/2,0],[1,(1+g^2)/2,(2+g^2)/2,(1+g^2)/2,(2+g^2)/2,repelem((1+v)/2,k),repelem((2+v)/2,k),repelem((1+v)/2,k),repelem((2+v)/2,k)],[(g^2)/2,(1+g^2)/2,(g^2)/2,(1+g^2)/2,alpha/2,(1+alpha)/2,alpha/2,(1+alpha)/2,beta/2,(1+beta)/2,beta/2,(1+beta)/2,repelem((v)/2,k),repelem((1+v)/2,k),repelem((v)/2,k),repelem((1+v)/2,k),0,0],[],arg(i));
    end
    cap_t_final(m,:) = cap_t;
end
