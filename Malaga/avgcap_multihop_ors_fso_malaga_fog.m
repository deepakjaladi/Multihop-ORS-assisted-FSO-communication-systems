    ...
    for i = 1:length(Eb_N0_dB)
        ...
        for d_1 = 1:beta
            for d_2 = 1:beta
                ...
                cap_t(i) = cap_t(i)+(const_1*const_2*b_d1*b_d2*meijerG([0],[1,(1+g^2)/2,(2+g^2)/2,(1+g^2)/2,(2+g^2)/2,repelem((1+v)/2,k),repelem((2+v)/2,k),repelem((1+v)/2,k),repelem((2+v)/2,k)],[(g^2)/2,(1+g^2)/2,(g^2)/2,(1+g^2)/2,alpha/2,(1+alpha)/2,alpha/2,(1+alpha)/2,d_1/2,(1+d_1)/2,d_2/2,(1+d_2)/2,repelem((v)/2,k),repelem((1+v)/2,k),repelem((v)/2,k),repelem((1+v)/2,k),0,0],[],arg(i)));
            end
        end
    end
    cap_t_final(m,:) = cap_t;
