...
for m = 1:2
    ...
    for i = 1:length(Eb_N0_dB)
        ...
        cap_t(i) = const_1*const_2*meijerG([0],[1,(1+g^2)/2,(2+g^2)/2,(1+g^2)/2,(2+g^2)/2,(1+v)/2,(1+v)/2,(2+v)/2,(2+v)/2,(1+v)/2,(1+v)/2,(2+v)/2,(2+v)/2],[(g^2)/2,(1+g^2)/2,(g^2)/2,(1+g^2)/2,alpha/2,(1+alpha)/2,alpha/2,(1+alpha)/2,beta/2,(1+beta)/2,beta/2,(1+beta)/2,v/2,v/2,(1+v)/2,(1+v)/2,v/2,v/2,(1+v)/2,(1+v)/2,0,0],[],arg(i));
    end
    cap_t_final(m,:) = cap_t;
end
...
figure
semilogy(Eb_N0_dB,cap_s_oall_final(1,:),'^',Eb_N0_dB,cap_s_oall_final(2,:),'square','linewidth',3,'markersize',8);
hold on
